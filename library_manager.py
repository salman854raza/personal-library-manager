...
def get_library_stats():
    ...
    decades_count = dict(sorted(decades_count.items(), key=lambda x: x[0]))
...
lottie_book = load_lottieurl("https://lottie.host/9b2bfbb3-2489-4f3d-9a5d-4cd854a135c5/6sEmUZT7nU.json")
if lottie_book:
    with st.sidebar:
...
if st.session_state.current_view == "add":
    ...
    if not st.session_state.library:
        st.markdown("<div class='warning-message'>Your library is empty. Add some books to see stats!</div>", 
                   unsafe_allow_html=True)
    else:
...
st.markdown("---")
st.markdown("Copyright © 2024 Salman Raza Personal Library Manager", unsafe_allow_html=True)
